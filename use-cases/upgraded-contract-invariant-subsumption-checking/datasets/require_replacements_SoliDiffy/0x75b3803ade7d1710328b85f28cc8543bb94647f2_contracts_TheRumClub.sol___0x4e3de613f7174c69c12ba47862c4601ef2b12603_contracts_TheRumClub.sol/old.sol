  ...
  ERC1155PausableUpgradeable
{
  ...
  function mintBatch(
    ...
    require(
      msg.value >= (p1 * PRICE_SMALL_PORTION + p2 * PRICE_LARGE_PORTION),
      'BAPC: Not enough funds'
    );

    address wallet = _msgSender();

    bytes32 hash = keccak256(abi.encodePacked(wallet, p1, p2, p3, timestamp));
    require(_verifier == _recoverSigner(hash, sig), 'BAPC: invalid signature');

    require(minters[wallet] == 0, 'BAPC: already minted');
